...
static void set_device_class(int hdev, char* devclass)
{

	int s = hci_open_dev(hdev);

        uint32_t cod = strtoul(devclass, NULL, 16);
...
void open_sock()
{
	...
	isg = l2cap_listen(BDADDR_ANY, L2CAP_PSM_HIDP_INTR, lm, 10);
        ...
        fprintf(stderr, "listened sock\n");

        ctrl = l2cap_accept(csg, NULL);
	
	intr = l2cap_accept(isg, NULL);	

        fprintf(stderr, "accepted sock\n");
